...
專案根目錄的 `AGENTS.md` 定義所有 agent 共用的驗證規則，`validation/` 則保存可透過 Git 在不同電腦間交換的測試證據。平台結果互相獨立：Mac MPS 不證明 Windows CUDA，Windows CUDA 也不證明 Mac MPS；報告只適用於其中記錄的精確 source commit。
...
若報告的 source commit 與目前 commit 不同，狀態會是 `stale`；缺少報告、dirty worktree、缺少工具或 requested accelerator 不可用時會是 `blocked`。工具只產生報告，不會自動 commit 或 push。Windows 與 Mac 的完整 Git handoff 與資料安全規則請參閱 [`validation/README.md`](validation/README.md)。
